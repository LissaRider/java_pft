...
public class ContactXMLDataTests extends TestBase {
  ...
  public void paramTestRUContactFromXmlCreation(ContactData contact) {
    app.goTo().homePage();
  ...
  @Test(testName = "EN Проверка создания контакта (XML)",
          dataProvider = "contactsENFromXmlFile", dataProviderClass = ContactDataProviders.class)
  public void paramTestENContactFromXmlCreation(ContactData contact) {
    app.goTo().homePage();
    Contacts before = app.contact().all();
    app.contact().create(contact);
    assertThat(app.contact().count(), equalTo(before.size() + 1));
